            ...
            for key, value in update["$pull"].items():
                if key in doc:
                    if isinstance(value, dict):
                        # MongoDB partial dict match: remove elements where all
                        # filter fields match (e.g. $pull: {assignedHardware: {hardwareId: x}})
                        doc[key] = [
                            item for item in doc[key]
                            if not all(item.get(k) == v for k, v in value.items())
                        ]
                    elif value in doc[key]:
...
def valid_user(fake_users):
    ...
    fake_users.insert_one({"userId": "test-user", "password": "test-password"})
    return "test-user"
